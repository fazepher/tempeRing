...
attempt_quanta <- function(mode_info, x_1, x_2, beta_1, beta_2, l_1, l_2, l_target, ...){
    ...
    if(delta_l > 0 || log(runif(1)) <= delta_l){
        return(list("acc" = TRUE, "beta_next" = c(beta_2, beta_1), "l_next" = swaped_l, "x_next" = c(x_quanta_1, x_quanta_2)))
    }
    return(list("acc" = FALSE, "beta_next" = c(beta_1, beta_2), "l_next" = c(l_1, l_2), "x_next" = c(x_1, x_2)))
